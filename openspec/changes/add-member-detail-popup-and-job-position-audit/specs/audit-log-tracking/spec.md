...
The system SHALL record login, agent create/update/delete actions, file upload/delete actions, permission changes, and employee `job_position` updates as append-only audit log entries.
...
#### Scenario: Employee job position update creates audit entry
- **WHEN** a user successfully updates an employee `job_position`
- **THEN** the system creates an audit log entry describing the employee update action and affected employee

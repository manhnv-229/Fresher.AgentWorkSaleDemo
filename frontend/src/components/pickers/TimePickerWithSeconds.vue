...
watch(
  ...
  (value) => {
    ...
    const parsedTime = parseTime(value, true);
    if (!parsedTime) {
      return;
    }
    ...
    pendingSeconds.value = String(parsedTime.seconds).padStart(2, '0');
  },
  { immediate: true }
);
...
function commitValue() {
  ...
  const nextValue = formatTime(date, true);
...
function selectSecond(option: string) {
  pendingSeconds.value = option;
}
